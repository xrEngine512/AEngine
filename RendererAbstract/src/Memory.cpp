...
namespace ShaderSystem
{
	inline size_t AlignedSize(size_t size, const int alignment = 16)
	{
		if (size % alignment != 0)
		{
			size_t aligned_size = (size / alignment) * alignment; //Optimizaton may affect
    ...
    Memory::Memory(Memory && lhr):
		m_Blocks(move(lhr.m_Blocks)),
		m_pData(lhr.m_pData),
		m_allocatedSize(lhr.m_allocatedSize),
		m_usedSize(lhr.m_usedSize)
	{
		lhr.m_pData = nullptr;
		lhr.m_allocatedSize = lhr.m_usedSize = 0;
    }

    void Memory::operator=(Memory && lhr) {
		if(&lhr != this) {
			m_Blocks = move(lhr.m_Blocks);
			m_pData = lhr.m_pData;
			m_allocatedSize = lhr.m_allocatedSize;
