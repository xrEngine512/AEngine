...
m_StatDisplay(nullptr), m_numberOfObjects(0)
{
	...
	m_Input->BindToKey(M_Key) += [=](void){ m_HUD2D->ShowMessage(L"Message", 300, 100); };
}
...
bool Scene::Initialize(HWND hwnd, int screenWidth, int screenHeight, bool FullScreen)
{
	bool result;
	m_hwnd = hwnd;
...
bool Scene::LoadThread(IInternalRenderObject* newRenderObject ,const char *modelFileName, float PosX, float PosY, float PosZ, float RotX, float RotY, float RotZ)
{
    ...
    {
        ApproxException ex(L"Could not initialize RenderObject");
        ex+=e;
        ex(m_hwnd);
...
{
    XMFLOAT3 tmppos = m_Camera->GetPosition();
    x = tmppos.x;
    y = tmppos.y;
    z = tmppos.z;
}
