...
namespace ShaderSystem
{
	...
	void AbstractShader::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename, unsigned short NumberOfTextureSlots, WCHAR* gsFileName)
	{
		...
		if (FAILED(result))
		{
			...
			if (errorMessage)
			{
				...
				throw ApproxException(string("Failed to compile Vertex Shader of ") + m_desc.name, "AbstractShader");
			}
			// If there was nothing in the error message then it simply could not find the shader file itself.

			throw ApproxException(string("Missing Vertex Shader File of ") + m_desc.name, "AbstractShader");
		}
		...
		if (FAILED(result))
		{
			...
			{
				OutputShaderErrorMessage(errorMessage, vsFilename);
				throw ApproxException(string("Failed to compile Pixel Shader of ") + m_desc.name, "AbstractShader");
			}
			// If there was nothing in the error message then it simply could not find the shader file itself.

			throw ApproxException(string("Missing Pixel Shader File of ") + m_desc.name, "AbstractShader");
		}
		if (gsFileName)
		{
			...
			if (FAILED(result))
			{
				...
				{
					OutputShaderErrorMessage(errorMessage, vsFilename);
					throw ApproxException(string("Failed to compile Geometry Shader of ") + m_desc.name, "AbstractShader");
				}
				// If there was nothing in the error message then it simply could not find the shader file itself.

				throw ApproxException(string("Missing Geometry Shader File of ") + m_desc.name, "AbstractShader");
			}
		}
		...
		if (FAILED(result))
		{
			throw ApproxException(string("CreateVertexShader Failed of ") + m_desc.name, "AbstractShader");
		}
		...
		if (FAILED(result))
		{
			throw ApproxException(string("CreatePixelShader Failed of ") + m_desc.name, "AbstractShader");
		}
		...
		if (geometryShaderBuffer)
		{
			...
			if (FAILED(result))
			{
				throw ApproxException(string("CreateGeometryShader Failed of ") + m_desc.name, "AbstractShader");
			}
		}
		if (!CreateInputLayout(device, vertexShaderBuffer))
		{
			throw ApproxException(string("Couldn't create Input layout of ") + m_desc.name, "AbstractShader");
		}
		...
		{
			throw ApproxException(string("Couldn't create Sampler state of ") + m_desc.name, "AbstractShader");
		}
		// Setup the description of the dynamic matrix constant buffer that is in the vertex shader.
		if (!SetupConstantBuffer(device))
		{
			throw ApproxException(string("Couldn't Setup Constant Buffer of ") + m_desc.name, "AbstractShader");
		}

		// Setup the description of the light dynamic constant buffer that is in the pixel shader.
		if (!SetupBuffers(device))
		{
			throw ApproxException(string("Couldn't Setup Buffers of ") + m_desc.name, "AbstractShader");
		}
	...
	bool AbstractShader::Render(ID3D11DeviceContext* context, unsigned int indexCount, ID3D11ShaderResourceView** textures, const GenericStruct*)
	{
		bool result;
