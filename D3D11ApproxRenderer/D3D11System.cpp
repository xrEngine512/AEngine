...
bool D3D11::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, 
			  float screenDepth, float screenNear)
{
    ...
    if (FAILED(m_device->CreateBlendState(&blendDesc, &m_d3dBlendStateAlpha)))
        return false;
    blendDesc.RenderTarget[0].BlendEnable = false;

    if (FAILED(m_device->CreateBlendState(&blendDesc, &m_d3dBlendStateDisabled)))
        return false;
    ...
    void D3D11::TurnZBufferOn()
    {
        m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
        return;
    }
    void D3D11::TurnZBufferOff()
    {
	...
	void D3D11::Shutdown()
{
    ...
    DELETE_D3D11_OBJECT(m_renderTargetView);
    DELETE_D3D11_OBJECT(m_deviceContext);
    DELETE_D3D11_OBJECT(m_device);
    DELETE_D3D11_OBJECT(m_swapChain);
    DELETE_D3D11_OBJECT(m_depthDisabledStencilState);
	return;
}
...
void D3D11::EnableAlphaBlending()
{
    m_deviceContext->OMSetBlendState(m_d3dBlendStateAlpha, 0, 0xffffffff);
}

void D3D11::DisableAlphaBlending()
{
...
void D3D11::GetProjectionMatrix(XMMATRIX& projectionMatrix)
{
...
void D3D11::GetWorldMatrix(XMMATRIX& worldMatrix)
{
...
void D3D11::GetOrthoMatrix(XMMATRIX& orthoMatrix)
{
