...
HLSLHighlighter::HLSLHighlighter(QObject *parent)
	...
	m_regExpKeywords("\\b(float|float2|float3|float4|matrix|min16float|min10float|min16int|min12int|min16uint|line|point|\
					 ...
					 |technique10|technique11|texture|Texture1D|Texture1DArray|Texture2D|Texture2DArray|\
...
void HLSLHighlighter::SetApproxVars(const std::vector<std::string> &vars)
{
	if (!m_regExpApproxVars)
	{
		...
		pattern += ")\\b";
		m_regExpApproxVars = new QRegExp(pattern.c_str());		//# ¬Œ«ÃŒ∆Õ¿ Õ≈œ–¿¬»À‹Õ¿ﬂ –¿¡Œ“¿ »À» Œÿ»¡ ¿ “. . ÀŒ ¿À‹Õ€… Œ¡⁄≈ “ ”Õ»◊“Œ∆¿≈“—ﬂ
	}
}
...
void HLSLHighlighter::SetApproxVars(const std::vector<QString>& vars)
{
	if (!m_regExpApproxVars)
	{
		...
		pattern.remove(pattern.size() - 1, 1);
		pattern += ")\\b";
