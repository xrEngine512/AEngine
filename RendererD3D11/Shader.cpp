...
{
	AbstractShader::AbstractShader(const wchar_t *acs_filename, const ShaderDesc& desc) :m_desc(desc), m_vertexShader(nullptr), m_pixelShader(nullptr),
	...
	void AbstractShader::LoadBuffers(ID3D11Device* device, const ShaderElement& elem, BufferPack& pack)
	{
		short num = 0;
	...
	void AbstractShader::Initialize(ID3D11Device* device)
	{
	...
	//if (FAILED(result)) throw ApproxException(L"������ ��� �������� Input layout");
	}
	...
	HRESULT AbstractShader::CreateInputLayoutDescFromVertexShaderSignature(LPCVOID bufPtr, SIZE_T bufSize, ID3D11Device* pD3DDevice, ID3D11InputLayout** pInputLayout, int* inputLayoutByteLength)
	{
		// Reflect shader info
	...
	void AbstractShader::InitializeBuffer(ID3D11Device* device, const ASL::RuntimeBufferInfo& info, BufferPack& pack, short BufferNum, const vector<int>& ParamIDs)
	{
	...
	bool AbstractShader::UpdateBuffer(ID3D11DeviceContext* context, ID3D11Buffer* buffer, MemoryMultiplexer *data)
	{
	...
	bool AbstractShader::UpdateSceneConstantsBuffers(ID3D11DeviceContext* context)
	{
	...
	bool AbstractShader::UpdatePerFrameBuffers(ID3D11DeviceContext* context)
	{
	...
	bool AbstractShader::UpdatePerObjectBuffers(ID3D11DeviceContext* context)
	{
	...
	bool AbstractShader::Render(ID3D11DeviceContext* context, unsigned int indexCount, ID3D11ShaderResourceView** textures, const GenericStruct* sets)
	{
		*m_ParamsData = *sets;
	...
	{
		OutputDebugStringW(L"AbstractShader Destroy\n");
	}
